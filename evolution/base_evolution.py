...
class BaseEvolution:
    ...
    def compute_generation(self, generators_population: Population, discriminators_population: Population):
        # store best of generation in coevolution memory
        self.evaluator.init_generation(self.generation)
        self.evaluator.update_bests(generators_population, discriminators_population)

        if self.generation >= config.evolution.max_generations - config.evolution.train_best_generations - 1:
            # reduce population to best G and best D
            generators_population = Population([generators_population.best()])
            discriminators_population = Population([discriminators_population.best()])
            self.evaluator.evaluate_population(
                generators_population.phenotypes(), discriminators_population.phenotypes(),
                batches_limit=config.gan.train_batches_limit)
        else:
