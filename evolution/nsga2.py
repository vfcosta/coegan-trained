...
class NSGA2(BaseEvolution):
    ...
    def next_population(self, generators_population, discriminators_population):
        ...
        previous_discriminators = discriminators_population.phenotypes()
        previous_generators = generators_population.phenotypes()
        self.evaluator.evaluate_population(g_children, previous_discriminators)
        self.evaluator.evaluate_population(previous_generators, d_children)

        self.evaluate_population(g_children + previous_generators, d_children + previous_discriminators)
        self.add_archive(g_children, self.archive_g)
        self.add_archive(d_children, self.archive_d)
        generators = self.nsga2_base.generate_next_population(previous_generators, g_children)
        discriminators = self.nsga2_base.generate_next_population(previous_discriminators, d_children)
    ...
    def add_archive(self, individuals, archive, max_size=500, prob=None):
        prob = prob or config.evolution.nslc.archive_prob
    ...
    def evaluate_population(self, generators, discriminators):
        ...
        for population, archive in [(generators, self.archive_g), (discriminators, self.archive_d)]:
            logger.debug(f"archive {len(archive)}")
            ...
            for i, p in enumerate(population):
                ...
                logger.debug(f"{i} {neighbors} {list(distances.values())} {fitness} {p.fitness()}")
                ...
                logger.debug(f"objectives {p.objectives}")
