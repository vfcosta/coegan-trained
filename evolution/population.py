...
class Population:
    ...
    def __init__(self, phenotypes, desired_species=1, speciation_threshold=None, stats={}):
        self.species_list = []
        self.speciation_threshold = speciation_threshold or config.evolution.speciation.threshold
    ...
    def adjust_speciation_threshold(self, phenotypes, max_runs=2):
        ...
        while len(self.species_list) != self.desired_species and self.speciation_threshold > 0 and runs < max_runs:
            ...
            logger.debug(f"distance {distance} {self.desired_species} {len(self.species_list)}")
        ...
        logger.debug(f"after adjust {self.desired_species} {len(self.species_list)}")
