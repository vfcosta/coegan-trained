...
from .non_evolutionary import NonEvolutionary
...
class GanTrain:
    ...
    def __init__(self, log_dir=None):
        ...
        algorithms = {"NEAT": NEAT, "NSGA2": NSGA2, "Lexicase": Lexicase, "NonEvolutionary": NonEvolutionary}
        self.evolutionary_algorithm = algorithms[config.evolution.algorithm](evaluator)
    ...
    def create_dataset(cls):
        ...
        if config.gan.dataset_resize:
            transform_arr.insert(0, transforms.Resize(config.gan.dataset_resize.to_container()))
        transform = transforms.Compose(transform_arr)
        base_path = os.path.join(os.path.dirname(__file__), "..", "data")
        logger.info("using dataset %s", config.gan.dataset)
    ...
    def start(self):
        if config.evolution.fitness.generator == "FID" or config.stats.calc_fid_score or config.stats.calc_fid_score_best:
            generative_score.initialize_fid(self.train_loader, size=config.evolution.fitness.fid_true_sample_size)
        ...
        for generation in tqdm(range(config.evolution.max_generations-1)):
            ...
            Discriminator.selected_loss = None
            Generator.selected_loss = None
